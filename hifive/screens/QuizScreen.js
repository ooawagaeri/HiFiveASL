...
function QuizScreen() {
      ...
      for (j = 0; j < image.length; j++) {
        if (ans[i].toUpperCase() === image[j].name) {
          holdLetters.push({
            id: i,
            title: image[j].name,
            url: "http://45-126-126-89.cloud-xip.io/media/" + image[j].image,
          });
        }
      }
    }
  ...
  const renderChoice = ({ item }) => <Choice option={item.choice} />;
  const Choice = ({ option }) => (
    <TouchableOpacity
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.prompt}>
          Key in the letter corresponding to this sign!
        </Text>
        <View style={{ alignItems: "center", flex: 1 }}>
          ...
          <View style={{ flex: 0.5, marginTop: 0 }}>
            <View>
              {marking === false ? (
                <Text style={styles.markingFalse}>Wrong answer!</Text>
              ) : marking === true ? (
